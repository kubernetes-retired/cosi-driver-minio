...
func (x *C) CreateBucket(ctx context.Context, bucketName string, options MakeBucketOptions) (string, error) {
	if err := x.client.minioClient.MakeBucket(ctx, bucketName, minio.MakeBucketOptions(options)); err != nil {
...
func (x *C) AddUser(ctx context.Context, bucket string) (*auth.Credentials, error){
	...
	if err := x.client.adminClient.AddUser(ctx, creds.AccessKey, creds.SecretKey); err != nil {
	...
	if err := x.client.adminClient.AddCannedPolicy(ctx, "s3:*", &p); err != nil {
		klog.Error("failed to add canned policy", err)
		return nil, err
	}

	if err := x.client.adminClient.SetPolicy(ctx, "s3:*", creds.AccessKey, false); err != nil {
